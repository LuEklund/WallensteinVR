...
pub const Context = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, extensions: []const [*:0]const u8, layers: []const [*:0]const u8) !Self {
        ...
        const dispatcher = try Dispatcher.init(instance);

        const debug_messenger: *c.XrDebugUtilsLabelEXT = @ptrCast(@alignCast(try createDebugMessenger(
            dispatcher,
            instance,
        )));
        ...
        try loader.xrCheck(c.xrGetSystem(instance, &system_info, &system_id));

        var system_properties = c.XrSystemProperties{ .type = c.XR_TYPE_SYSTEM_PROPERTIES, .next = null };
        try loader.xrCheck(c.xrGetSystemProperties(instance, system_id, &system_properties));
        ...
        // _ = try getVulkanInstanceRequirements(
        //     dispatcher,
        //     allocator,
        //     instance,
        //     system_id,
    ...
    pub fn deinit(self: Self) void {
        _ = self.dispatcher.xrDestroyDebugUtilsMessengerEXT(@ptrCast(self.debug_messenger)) catch {};
...
fn validateExtensions(allocator: std.mem.Allocator, extentions: []const [*:0]const u8) !void {
    ...
    try loader.xrCheck(c.xrEnumerateInstanceExtensionProperties(null, 0, &extension_count, null));
    ...
    try loader.xrCheck(c.xrEnumerateInstanceExtensionProperties(null, extension_count, &extension_count, @ptrCast(extension_properties.ptr)));
...
pub fn validateLayers(allocator: std.mem.Allocator, layers: []const [*:0]const u8) !void {
    ...
    try loader.xrCheck(c.xrEnumerateApiLayerProperties(0, &layer_count, null));
    ...
    try loader.xrCheck(c.xrEnumerateApiLayerProperties(layer_count, &layer_count, layer_properties.ptr));
...
pub fn getVulkanInstanceRequirements(
    ...
    instance: c.XrInstance,
    system: c.XrSystemId,
) !struct { graphics_requirements: c.XrGraphicsRequirementsVulkanKHR, extensions: []const [*:0]const u8 } {
    ...
    try dispatcher.xrGetVulkanInstanceExtensionsKHR(
        instance,
        system,
        0,
        &instance_extensions_size,
        null,
    );
    ...
    var extensions = try std.ArrayList([*:0]const u8)
        .initCapacity(allocator, instance_extensions_size + 1);
    ...
    return .{
        .graphics_requirements = graphics_requirements,
...
pub fn getVulkanDeviceRequirements(
...
) !struct { physical_device: c.VkPhysicalDevice, extensions: []const [*:0]const u8 } {
    ...
    try dispatcher.xrGetVulkanDeviceExtensionsKHR(
        instance,
        system,
        0,
    ...
    var extensions = try std.ArrayList([*:0]const u8)
        .initCapacity(allocator, device_extensions_size);
