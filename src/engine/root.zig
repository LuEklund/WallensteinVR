...
var near_block: [2]i32 = .{ -1, -1 };var blocks: std.ArrayList(input.Block) = undefined;
...
pub const Engine = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, config: Config) !Self {
        ...
        const window: sdl.video.Window = try .init("Hello SDL3", window_widthh, window_height, .{ .vulkan = true, .resizable = true });
        try window.show();
    ...
    pub fn start(self: Self) !void {
        ...
        var vulkan_swapchain: VulkanSwapchain = try .init(self.vk_physical_device, self.vk_logical_device, self.sdl_surface, window_widthh, window_height;
